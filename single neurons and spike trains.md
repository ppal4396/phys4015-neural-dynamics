...
$$C_m \frac{dV_m}{dt} = {G}^{max}_K n^4 (V_m - E_K) + {G}^{max}_{Na} m^3 h (V_m - E_{Na}) + {G}^{max}_L (V_m - E_L) + I_{app}$$
...
However note real neural spike trains often show greater irregularity than expected from a Poisson point process.
...
This function is the simplest way to represent response to a stimulus over time. It represents spike times with value 1 and non-spiking times with value 0.

Formally, we have a set of spike times, after some stimulus. 
$$S = \{t_1, t_2, \ldots , t_n \}$$
**Neural response** at time $t$ is found by summing the Dirac delta function of times since previous spikes.
$$\rho (t) = \sum_{i=1}^{n}\delta(t - t_i)$$
i.e. this yields $\rho(t) = 1$ if there is a $t_i = t$ and $0$ otherwise since the neuron will only spike once at any point in time.

### three notions of 'firing rate'

#### spike count rate

Describes the firing rate of one neuron over some period of time.

Counts the number of spikes in a time interval, divided by time interval. Formally, this is the neural response averaged over time.
$$r = \frac{n}{T} = \frac{1}{T}\int_0^{T}\rho(\tau)\,d\tau$$
#### firing rate (or 'spike density')

A notion of mean firing rate arrived at across trials for one neuron.
...
A notion of mean firing rate arrived at across neurons.

In each time bin (or as written below for one time window), you find average spike count across population, instead of across trials.
...
A plot that estimates the 'mean response' of a neuron to a stimulus.
